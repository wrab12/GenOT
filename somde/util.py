...
def regress_out(sample_info, expression_matrix, covariate_formula,
                design_formula='1', rcond=-1):

    from numpy.linalg import LinAlgError
    if 'total_count' in sample_info.columns:
        sample_info = sample_info.copy()
        sample_info['total_count'] = np.where(
            sample_info['total_count'] <= 0,
            1e-5,
            sample_info['total_count']
        )



    covariate_matrix = patsy.dmatrix(covariate_formula + ' - 1', sample_info)
    design_matrix = patsy.dmatrix(design_formula, sample_info)

    design_batch = np.hstack((design_matrix, covariate_matrix))


    rank = np.linalg.matrix_rank(design_batch)

    try:
        coefficients, res, rank, s = np.linalg.lstsq(
            design_batch, expression_matrix.T, rcond=rcond
        )
    except LinAlgError as e:
        Q, R = np.linalg.qr(design_batch)
        coefficients = np.linalg.solve(R, Q.T @ expression_matrix.T)
